...
local enc = 1
local encRate = 7 --change this depending on area

function encPRNG(currseed)
  newcurrseed = currseed * 0x41C64E6D + 0x3039

  return newcurrseed
end

function encCheck(newcurrseed)
print(newcurrseed % 1600)
  if newcurrseed % 1600 > (16 * encRate) then
    enc = 0
  else enc = 1
...
while true do
  local currseed = mdword(0x20386D0)
  encPRNG(currseed)
  encCheck(newcurrseed)
  local frame = vba.framecount()
  gui.text(5, 130, "Frame: " ..string.format(frame))
  gui.text(5, 138, "EncCurrSeed: " ..string.format("%08X", currseed))
  gui.text(5, 146, "NextEncCurrSeed:" ..string.format("%08X", newcurrseed))
  gui.text(5, 153, "EncCheck: " ..string.format(enc))

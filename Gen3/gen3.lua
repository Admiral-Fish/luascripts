...
personality = mdword(party)
trainerid = mdword(party + 4)
magicword = xor(personality, trainerid)
seed = mdword(0x03004818)
i = personality%24
...
species=band(xor(mdword(party + 32 + growthoffset), magicword), 0xFFF)
level = party + 84

ivs=xor(mdword(party + 32 + miscoffset + 4), magicword)
...
hidpowbase = math.floor(((band(hpiv,2)/2 + band(atkiv,2) + 2*band(defiv,2) + 4*band(spdiv,2) + 8*band(spatkiv,2) + 16*band(spdefiv,2))*40)/63 + 30)
...
function wildview()
	...
	magicword = xor(personality, trainerid)
	ivs = xor(mdword(wild + 32 + miscoffset + 4), magicword)
	hpiv = band(ivs, 0x1F)
	atkiv = band(ivs, 0x1F * 0x20) / 0x20
	defiv = band(ivs, 0x1F * 0x400) / 0x400
	spdiv = band(ivs, 0x1F * 0x8000) / 0x8000
	...
	species = band(xor(mdword(wild + 32 + growthoffset), magicword), 0xFFF)
